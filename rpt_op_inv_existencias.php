...
echo "<script language='JavaScript'>
		...
		{	var rpt_territorio, rpt_almacen, tipo_item, rpt_ver, rpt_fecha, rpt_ordenar, rpt_grupo, rpt_formato, rpt_barcode,rpt_tipo,rpt_modelo;
...
echo "<input type='hidden' id='tipo' name='tipo' value='$tipo'>";
	...
	$sql="select codigo, nombre from grupos where estado=1 and cod_tipo=".$tipo." order by 2";
	$resp=mysqli_query($enlaceCon,$sql);
	while($dat=mysqli_fetch_array($resp))
	{	$codigo=$dat[0];
		$nombre=$dat[1];
