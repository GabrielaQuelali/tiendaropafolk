...
do {
	...
	if($tipoDoc==1 || $tipoDoc==4){
		...
		$sql_insert="insert into `salida_almacenes`(`cod_salida_almacenes`, `cod_almacen`,`cod_tiposalida`, 
			...
			'$observaciones', '1', '$nro_correlativo', 1, '$codCliente', '$totalVenta', '$descuentoVenta', '$totalFinal', '$razonSocial', 
	...
	if($tipoSalida==1001){
		...
		if($tipoDoc==1){		
			...
			if($banderaCorreo==1 || $banderaCorreo==2){
				...
				echo "<script type='text/javascript' language='javascript'>
				location.href='formatoNotaRemisionOnLine.php?codVenta=$codigo';
				</script>";		
			}
		}
	}else{	
