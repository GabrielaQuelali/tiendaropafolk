...
while ($datos = mysqli_fetch_array($resp)) {
	...
	if($tipoAgrupacion == 'GRUPO'){
	    ...
	    where s.`fecha` BETWEEN '$fecha_iniconsulta' and '$fecha_finconsulta'
	    and s.`salida_anulada`= 1 and s.`cod_tiposalida`=1001 and  
	    s.`cod_almacen` in (select a.`cod_almacen` from `almacenes` a where a.`cod_ciudad` in ($rptTerritorioString) ) and pc.cod_producto_costo='$codProductoFinal' ";
		if($rpt_ver==1){
			$sqlPrecios.=" and pc.costo_si_no = 1 ";	
		} 
	    ...
	    if($rpt_ver==1){
			$sqlPrecios.=" and m.costo_si_no=1 ";	
		} 
	...
	/*GANANCIA*/
	$gananciaUnitariaF = formatonumeroDec($precioPromedio - $costoUnitarioProducto);
	$gananciaTotalF = formatonumeroDec($montoVentaProducto - $costoTotalProducto);

	$totalSumaGanancias = $totalSumaGanancias + ($montoVentaProducto - $costoTotalProducto);
	...
	echo "<tr>
		...
		<td align='right' class='monto-venta' style='border: gray 1px solid; background-color: LightPink'>$montoVentaProductoF</td>
		<td align='right' title='$titlePreciosDetalle'>$precioPromedioF</td>
		<td align='right'>
		...
		<td align='right' class='costo-insumo-unitario' ><a href='#' onclick='mostrarInsumosProductos($codProductoFinal,\"$tipoAgrupacion\",$jsonInsumosEncode);'>$costoInsumosF</a></td>	
		<td align='right' class='costo-insumo-total'>$costoInsumosTotalF</td>	
		<td align='right' class='costo-proceso-unitario'><a href='#' onclick='mostrarProcesosProductos($codProductoFinal,\"$tipoAgrupacion\",$jsonProcesosEncode)'>$costoProcesosPromedioF</td>	
		<td align='right' class='costo-proceso-total'>$costoProcesosTotalF</td>	
		<td align='right' class='gasto-distribuido'>$montoGastoDistribuidoF</td>	
		<td align='right' class='costo-unitario' >$costoUnitarioProductoF</td>	
		<td align='right' class='costo-total' style='border: gray 1px solid; background-color: LightPink'>$costoTotalProductoF</td>	
		<td align='right' class='ganancia-unitaria' >$gananciaUnitariaF</td>	
		<td align='right' class='ganancia-total' >$gananciaTotalF</td>	
		<td align='right' >$margenProductoF</td>	
		<td align='right' >
...
<script>
	$(document).ready(function() {
		 ...
		 ***********************************************/
		// 
		$('#detalleCosto').on('keyup', '.cantidad-input, .precio-promedio-input', function() {
			...
			// * Ganancias
			var v_gananciaUnitaria = formatNumber ( (precioPromedio) - (costoUnitario) );
			$row.find('.ganancia-unitaria').text(v_gananciaUnitaria);

			var v_gananciaTotal = formatNumber ( (cantidad * precioPromedio) - (cantidad * costoUnitario) );
			$row.find('.ganancia-total').text(v_gananciaTotal);

			//console.log('hola1'+v_gananciaTotal);
		...
		function obtieneTotalesFinal(){
			...
			$('#detalleCosto .monto-venta').each(function() {
				var valor = parseFormattedNumber($(this).text());
				totalMontoVenta += valor;
			});
			$('#total-monto-venta').text(formatNumber(totalMontoVenta));
			// Sumar los valores de la columna Costo Total
			var totalSumaCosto = 0;
			$('#detalleCosto .costo-total').each(function() {
				var valor = parseFormattedNumber($(this).text());
				totalSumaCosto += valor;
			});
			$('#total-suma-costo').text(formatNumber(totalSumaCosto));
			// Sumar los valores de la columna Ganancia Total
			var GananciaTotal = 0;
			$('#detalleCosto .ganancia-total').each(function() {
				var valor = parseFormattedNumber($(this).text());
				GananciaTotal += valor;
			});
			$('#ganancia-total-final').text(formatNumber(GananciaTotal));
			...
			$('#detalleCosto .monto-venta').each(function() {
				...
				// Costo Unitario
				let c_insumo  = $(this).closest('tr').find('.costo-insumo-unitario').text();
				let c_proceso = $(this).closest('tr').find('.costo-proceso-unitario').text();
				let cantidad  = $(this).closest('tr').find('.cantidad-input').val();
				let costoUnitario = (parseFloat(c_insumo) + 
									parseFloat(c_proceso) + 
									(parseFloat(gastoDistribuido) / cantidad)).toFixed(2);
				$(this).closest('tr').find('.costo-unitario').text(formatNumber(costoUnitario));
			});
			// console.log('hola2');
		}
