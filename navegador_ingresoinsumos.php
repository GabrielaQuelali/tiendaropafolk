...
function ajaxBuscarIngresos(f){
	...
	var fechaIniBusqueda, fechaFinBusqueda, notaIngreso, verBusqueda, global_almacen, provBusqueda,tipo,estado;
	...
	ajax.open("GET", "ajaxNavIngresosInsumos.php?tipo="+tipo+"&estado="+estado+"&fechaIniBusqueda="+fechaIniBusqueda+"&fechaFinBusqueda="+fechaFinBusqueda+"&notaIngreso="+notaIngreso+"&global_almacen="+global_almacen+"&provBusqueda="+provBusqueda,true);
...
{   
	var estado,tipo;
			...
			tipo=f.tipo.value;
			location.href='registrar_ingresoinsumos.php?tipo='+tipo+'&estado='+estado;
}
    ...
    {   if(j==0)
        ...
        {      
                funOk(j_cod_registro,function(){
                    location.href='editar_ingresoInsumos.php?codIngreso='+j_cod_registro+'&tipo='+tipo+'&estado='+estado;
                });
        }
    }
}
...
var estado,tipo;
    ...
    {   if(j==0)
        ...
        {   //window.open('anular_ingreso.php?codigo_registro='+j_cod_registro+'&grupo_ingreso=2','','scrollbars=yes,status=no,toolbar=no,directories=no,menubar=no,resizable=yes,width=280,height=150');
                funOk(j_cod_registro,function(){
                    location.href='anular_ingresoInsumo.php?codigo_registro='+j_cod_registro+'&tipo='+tipo+'&estado='+estado;
                });
        }
    }
}
    ...
    function cambiar_vista(f)
		{
			...
			location.href='navegador_ingresoinsumos.php?estado='+estado+'&tipo='+tipo;
		}
        </script>
    </head>
    <body>
...
$consulta = "SELECT i.cod_ingreso_almacen, i.fecha, i.hora_ingreso, ti.nombre_tipoingreso,
 i.observaciones, i.nota_entrega, i.nro_correlativo, i.ingreso_anulado,p.nombre_proveedor,
	i.nro_factura_proveedor, i.created_by,i.created_date, i.modified_by, i.modified_date,es.nombre_estado 
