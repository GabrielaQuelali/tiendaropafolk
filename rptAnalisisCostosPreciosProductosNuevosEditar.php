...
$rpt_territorio = $_COOKIE['global_almacen'];
$rptTerritorioString = $rpt_territorio;


// $rptProductosString = implode(",", $rpt_productos);
$idanalisis = $_GET['idanalisis'];
$sqlProductos = "SELECT codigo_producto FROM analisis_costos_nuevos_detalle WHERE analisis_costo_id = '$idanalisis'";
$resultado = mysqli_query($enlaceCon, $sqlProductos);
$rptProductosArray = [];
while ($row = mysqli_fetch_assoc($resultado)) {
	$rptProductosArray[] = $row['codigo_producto'];
}
$glosa = '';
$sqlGlosa = "SELECT glosa FROM analisis_costos_nuevos WHERE id = '$idanalisis' LIMIT 1";
$resGlosa = mysqli_query($enlaceCon, $sqlGlosa);
if ($rowGlosa = mysqli_fetch_assoc($resGlosa)) {
	$glosa = $rowGlosa['glosa'];
}
$rptProductosString = implode(',', $rptProductosArray);
...
?>
<input type="hidden" name="codigoAnalisis" value="<?=$idanalisis?>">
<?php
...
echo "<table align='center' class='textotit' width='100%'><tr><td align='center'>Analisis de Costos y Precios
	...
	<br>Glosa: <input type='text' class='texto' name='glosa' id='glosa' size='50' value='".$glosa."'>
	</tr></table>";
...
while ($datos = mysqli_fetch_array($resp)) {
    ...
    // Obtener los datos del análisis de costos
    $sqlAnalisis = "SELECT 
						d.id,
						d.analisis_costo_id,
						d.codigo_producto,
						...
						d.costo_insumos,
						d.costo_procesos,
						d.total_costo_directo,
						d.cantidad_producir,
						d.horas_producir,
						d.porcentaje_costodirecto,
						d.porcentaje_cantidadproducir,
						d.porcentaje_costoindirecto,
						d.costo_indirectounitario,
						d.costo_unitariototal,
						d.precio_consignacion_sf,
						d.precio_ventadirecta_sf,
						d.precio_sugerido_sf,
						d.created_at,
						d.updated_at
                    FROM analisis_costos_nuevos_detalle d
                    WHERE d.analisis_costo_id = '$idanalisis'
                    AND d.codigo_producto = '$codProductoFinal'";
    $resAnalisis = mysqli_query($enlaceCon, $sqlAnalisis);
    $rowAnalisis = mysqli_fetch_array($resAnalisis);

    // Si existe análisis, obtener los valores; si no, usar 0
    $costoInsumos 		= $rowAnalisis['costo_insumos'] ?? 0;
    $costoProcesos 		= $rowAnalisis['costo_procesos'] ?? 0;
    $totalCostoDirecto 	= $rowAnalisis['total_costo_directo'] ?? 0;
    $cantidadProducir 	= $rowAnalisis['cantidad_producir'] ?? 0;
    $horasProducir 		= $rowAnalisis['horas_producir'] ?? 0;

    $porcCostoDirecto 		= $rowAnalisis['porcentaje_costodirecto'] ?? 0;
    $porcCantidadProducir 	= $rowAnalisis['porcentaje_cantidadproducir'] ?? 0;

    $costoIndirectoUnitario = $rowAnalisis['costo_indirectounitario'] ?? 0;
    $costoTotalUnitario 	= $rowAnalisis['costo_unitariototal'] ?? 0;

    $precioConsignacion = $rowAnalisis['precio_consignacion_sf'] ?? 0;
    $precioVentaDirecta = $rowAnalisis['precio_ventadirecta_sf'] ?? 0;
    $precioSugerido 	= $rowAnalisis['precio_sugerido_sf'] ?? 0;
	echo "<tr>
		...
		<td align='center' style='border: gray 1px solid; background-color: LightSalmon'>
			<input type='number' class='cantidad-producir' name='cantidad-producir[]' value='$cantidadProducir' style='width: 10ch;'>
		</td>

		<td align='center' style='border: gray 1px solid; background-color: LightSalmon'>
			<input type='number' class='horas-produccion' name='horas-produccion[]' value='$horasProducir' style='width: 10ch;'>
		</td>
		...
		<td style='display:none;'>
			<input type='hidden' class='costo_indirecto_distribuido' name='costo_indirecto_distribuido[]' value='$costoIndirectoDistribuido' style='width: 10ch;'>
		</td>
		<td align='right' class='costo-indirecto-distribuido'>$costoIndirectoDistribuido</td>
		
		<td style='display:none;'>
			<input type='hidden' class='costo_indirecto_unitario' name='costo_indirecto_unitario[]' value='$costoIndirectoUnitario' style='width: 10ch;'>
		</td>
		<td align='right' class='costo-indirecto-unitario'>$costoIndirectoUnitario</td>
		
		<td style='display:none;'>
			<input type='hidden' class='costo_total_unitario' name='costo_total_unitario[]' value='$costoTotalUnitario' style='width: 10ch;'>
		</td>
		<td align='right' class='total-costo-unitario'>$costoTotalUnitario</td>

		<td align='center' style='border: gray 1px solid; background-color: LightBlue'>
			<input type='number' class='precio-consignacion-sf' name='precio-consignacion-sf[]' value='$precioConsignacion' style='width: 10ch;' step='0.01'>
		</td>
		
		<td align='center' style='border: gray 1px solid; background-color: LightBlue' class='margen-consignacion-sf'>$margenConsignacion</td>

		<td align='center' style='border: gray 1px solid; background-color: LightBlue'>
			<input type='number' class='precio-ventadirecta-sf' name='precio-ventadirecta-sf[]' value='$precioVentaDirecta' style='width: 10ch;' step='0.01'>
		</td>
		
		<td align='center' style='border: gray 1px solid; background-color: LightBlue' class='margen-ventadirecta-sf'>$margenVentaDirecta</td>

		<td align='center' style='border: gray 1px solid; background-color: LightBlue'>
			<input type='number' class='precio-sugerido-sf' name='precio-sugerido-sf[]' value='$precioSugerido' style='width: 10ch;' step='0.01'>
		</td>
		
		<td align='center' style='border: gray 1px solid; background-color: LightBlue' class='margen-sugerido-sf'>$margenSugerido</td>
	</tr>";
...
<script>	
	$(document).ready(function() {
		...
		$('#detalleCosto').on('keyup', '.precio-consignacion-sf', function() {
			...
			// Obtiene Datos
			var precioConsignacionSF = parseFormattedNumber($row.find('.precio-consignacion-sf').val()) || 0;
			var totalCostoUnitario = parseFormattedNumber($row.find('.total-costo-unitario').text().trim());
		...
		// Función async para ejecutar triggers con delay
		async function triggerConDelay() {
			$(".cantidad-producir").each(function() {
				$(this).trigger("input");
			});
			await sleep(100);
			...
			await sleep(200);
			$(".precio-consignacion-sf").each(function () {
				$(this).trigger("keyup");
			});
			await sleep(200);
			$(".precio-ventadirecta-sf").each(function () {
				$(this).trigger("keyup");
			});
			await sleep(200);
			$(".precio-sugerido-sf").each(function () {
				$(this).trigger("keyup");
			});
		}
		// Ejecutar al final del ready
		triggerConDelay();
	});
</script>